...
define('AGORA_PAYULATAM_CURRENCY_COP', 'COP');	// set colompian currency to a global var for confirming purpose
define('AGORA_PAYULATAM_MINIMUM_AMMOUNT_COP', '5659');	// define payulatam minimum ammoung for COP
...
define('AGORA_PURCHASE_METHOD_PAYULATAM', 'PayU Latam');	// PayU Latam method of purchase
...
//		elgg_load_library('elgg:amap_maps_api_geocoder'); 
    ...
    elgg_register_css('agora_tooltip_css', '//code.jquery.com/ui/1.11.1/themes/smoothness/jquery-ui.css');
    elgg_register_css('agora_jrating_css', elgg_get_site_url() . 'mod/agora/assets/jRating.jquery.css');
    
    // Extend js
    elgg_register_js('paypal', elgg_get_site_url() . 'mod/agora/assets/paypal-button.min');  // paypal button  
    elgg_register_js('agorajs', elgg_get_site_url() . 'mod/agora/assets/agora.js');    
    // elgg_register_js('agora_tooltip_js', '//code.jquery.com/ui/1.11.1/jquery-ui.js'); // OBS
    elgg_register_js('agora_jrating_js', elgg_get_site_url() . 'mod/agora/assets/jRating.jquery.js'); 
    elgg_register_js('agora_map_search_js', elgg_get_site_url() . 'mod/agora/views/default/js/agora_map_search.php');
    ...
    elgg_register_action('agora/admin/payulatam_options', "$action_path/admin/settings.php", 'admin');
	...
	if (comrat_only_buyers_enabled()) {
		...
		foreach ($getsales as $gs) {
			...
			if (!check_if_user_commented_this_ad($entity->guid, $gs->txn_buyer_guid)) {
				notify_user($gs->txn_buyer_guid,
					$notifier_guid,
					elgg_echo('agora:comments:notify:subject'),
					elgg_echo('agora:comments:notify:body', array($entity->title, $entity->getURL()))
				);		
			}
		}
