...
*/
    ...
    if ($entity->save()) {
		// save the final price as metadata
        $entity->price_final = $entity->get_ad_price_with_shipping_cost();
        $entity->save();
        
		// save ad coords location and if amap_maps_api is enabled
		if (elgg_is_active_plugin("amap_maps_api")){
			if ($location) {
				$ccc = amap_ma_save_object_coords($location, $entity, 'amap_maps_api');
			}
		}
