...
if ($entity) {
    ...
    $user_purchases = check_if_user_purchased_this_ad($entity->guid, $user_logged_in->guid);
    ...
    if (comrat_only_buyers_enabled()) { // reviews and ratings enabled only for buyers
        if (elgg_instanceof($user_purchases[0], 'object', 'agorasales') && !check_if_user_commented_this_ad($entity->getGUID(), $user_logged_in->guid) && check_comrat_time($user_purchases[0]->time_created)) {
...
//            $content .= agora_elgg_view_comments($entity, true, $comment_vars);   // allow add review
        } else {
            $content .= RatingsOptions::ratings_elgg_view_comments($entity, false);	// disable review form 
//            $content .= agora_elgg_view_comments($entity, false, $comment_vars);  // disable review form
        }
    } else { // reviews and ratings enabled for all members
        if ($entity->comments_on != 'Off') {
            $content .= elgg_view_comments($entity, true, $comment_vars);
        }
    }

    // add download button if current user has purchased this item or if file is available for free
    if ($entity->digital && (elgg_instanceof($user_purchases[0], 'object', 'agorasales') || !$entity->price || elgg_is_admin_logged_in())) {
...
if (elgg_is_logged_in() && $entity) {
    ...
    if ($user && $user->guid == $entity->owner_guid) {
        ...
        $options = array(
            ...
            'metadata_name_value_pairs' => array(
                array('name' => 'txn_vguid', 'value' => $entity->guid, 'operand' => '='),
            ),
        );
        ...
        $options_int = array(
            ...
            'limit' => 0,
            'metadata_name_value_pairs' => array(
                array('name' => 'int_ad_guid', 'value' => $entity->guid, 'operand' => '='),
            ),
        );
        ...
        if ($interestlist) {
            ...
            foreach ($interestlist as $b) {
                ...
                if ($b->int_status == AGORA_INTEREST_INTEREST && ($entity->howmany > 0 || !is_numeric($entity->howmany))) {
